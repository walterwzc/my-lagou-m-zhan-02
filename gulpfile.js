...
gulp.task('packjs', function() {
    ...
    // 只有在 gulp 之中return 了task， 才能够保证后面的 gulpSequence之中，task 能够按照 并行、或者串行进行；
...
gulp.task('copypublic', function() {
    gulp.src('./src/public/**/*')
        .pipe(gulpif(option.env !== 'production', gulp.dest('./dev/public')))
        .pipe(gulpif(option.env === 'production', gulp.dest('./dist/public')))
})
...
gulp.task('watch', function() {
    ...
    watch(['./src/scripts/**/*.js', './src/templates/*.html'], function() {
        gulp.start('packjs')
    })
})
...
if (option.env !== 'production') {
    gulp.task('default', ['copyhtml', 'copymock', 'copystatic', 'copypublic', 'sass', 'packjs', 'webserver', 'watch'], function() {
        console.log('done.')
    })
} else {
    // gulp.task('default', ['clear', 'copystatic', 'copypublic', 'sass', 'packjs'], function () {
    //   // gulp.start('copyhtml')
    //   console.log('done.')
    // })
    gulp.task('default', function(cb) {
        gulpSequence('clear', ['copystatic', 'copypublic', 'sass', 'packjs'], 'copyhtml')(cb)
    })
}
